...
//        var tree1 = sim.tree(new Vec2(width / 4, height - 120), 5, 70, 0.95, (Math.PI / 2) / 3);
        var tree2 = sim.tree(new Vec2(width - width / 2, height), 5, 15, 0.95, (Math.PI / 2) / 3);
        ...
        tree2.drawParticles = function (ctx, composite) {
            ...
            for (i = 0; i < composite.particles.length; ++i) {
                ...
                if (particle.leaf) {
                    ...
                    ctx.beginPath();
                    ctx.arc(particle.pos.x, particle.pos.y, 3, 0, 2 * Math.PI);
        ...
        tree2.drawConstraints = function (ctx, composite) {
            ...
            for (i = 0; i < composite.constraints.length; ++i) {
                ...
                ctx.lineWidth = lerp(3, 2, constraint.p);
                ctx.stroke();
            }
    ...
    function treeBig() {
        var canvas = document.getElementById("scratchTree");
        ...
        tree2.drawParticles = function (ctx, composite) {
            ...
            for (i = 0; i < composite.particles.length; ++i) {
                ...
                if (particle.leaf) {
                    ...
                    ctx.beginPath();
                    ctx.arc(particle.pos.x, particle.pos.y, 15, 0, 2 * Math.PI);
                    ctx.fillStyle = "#679d7c";
                    ctx.fill();
                }
            }
